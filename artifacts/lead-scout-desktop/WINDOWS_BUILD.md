...
Для подписанного релиза добавьте в секреты GitHub `CSC_LINK` (или `WIN_CSC_LINK`)
и `CSC_KEY_PASSWORD`. Electron Builder использует эти значения для подписи
установщика и созданного при установке деинсталлятора. Workflow проверяет
Authenticode у обоих файлов, сохраняет `authenticode-report.json` с результатом и
данными сертификата и не публикует теговый релиз при отсутствии действительной
подписи. Ручной запуск без настроенного сертификата допускается только как
development-only прогон: отчет будет явно помечен, а его артефакты нельзя
распространять.
