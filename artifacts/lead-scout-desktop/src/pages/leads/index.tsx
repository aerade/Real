  ...
  return (
    <AppLayout>
      <div className="flex h-full min-h-0 flex-col gap-5">
        <header className="shrink-0 border-b border-border/70 pb-5"><p className="mb-2 flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.2em] text-primary"><BriefcaseBusiness className="h-3.5 w-3.5" />Личное рабочее пространство</p><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end"><div><h1 className="text-2xl font-semibold tracking-tight">Ваша воронка</h1><p className="mt-1 text-sm text-muted-foreground">Компании в вашей работе, а не весь каталог.</p></div><Link data-testid="link-clients-search" href="/search" className="inline-flex h-9 items-center justify-center gap-2 rounded-md border border-border/70 bg-card px-3 text-xs font-semibold hover:border-primary/50"><Search className="h-3.5 w-3.5" />Найти ещё лид</Link></div></header>
        ...
        <section className="flex min-h-0 flex-1 flex-col overflow-hidden rounded-xl border border-border/70 bg-card shadow-sm">
          ...
          <div className="min-h-0 flex-1 overflow-y-auto">
            {isLoading ? <div className="space-y-3 p-4">{[1, 2, 3, 4].map((item) => <div key={item} className="h-20 animate-pulse rounded-lg bg-muted/50" />)}</div>
              ...
              : <div className="divide-y divide-border/50"><div className="sticky top-0 z-10 hidden grid-cols-[2fr_1.2fr_1fr_1fr_70px] gap-4 border-b border-border/60 bg-card/95 px-4 py-2.5 text-[10px] font-bold uppercase tracking-[0.14em] text-muted-foreground backdrop-blur sm:grid"><span>Компания</span><span>Контекст</span><span>Этап</span><span>Обновлено</span><span className="text-right">Приоритет</span></div>{visibleLeads.map((lead) => { const leadStatus = statusMap[lead.status] ?? { label: lead.status, variant: "neutral" }; return <Link data-testid={`link-client-${lead.id}`} key={lead.id} href={`/leads/${lead.id}`} className="group grid gap-3 px-4 py-4 transition-colors hover:bg-accent/35 sm:grid-cols-[2fr_1.2fr_1fr_1fr_70px] sm:items-center sm:gap-4"><div className="min-w-0"><p data-testid={`text-client-name-${lead.id}`} className="truncate text-xs font-semibold">{lead.name}</p><p className="mt-1 truncate text-[10px] text-muted-foreground">{websiteHost(lead.website) ?? "Сайт не указан"}</p></div><div className="min-w-0"><p className="truncate text-xs text-foreground/85">{lead.industry}</p><p className="mt-1 truncate text-[10px] text-muted-foreground">{lead.city}</p></div><div><span data-testid={`status-client-${lead.id}`} className="inline-flex rounded-full border border-border/70 bg-background px-2 py-1 text-[10px] text-muted-foreground">{leadStatus.label}</span></div><div className="text-[10px] text-muted-foreground">{new Date(lead.updatedAt).toLocaleDateString("ru-RU", { day: "numeric", month: "short" })}</div><div className={cn("text-left font-mono text-sm font-semibold sm:text-right", lead.score >= 80 ? "text-emerald-300" : lead.score >= 50 ? "text-cyan-300" : "text-rose-300")}>{lead.score}</div></Link>; })}</div>}
          </div>
        </section>
      </div>
    </AppLayout>
